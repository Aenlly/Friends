...
import com.example.friends.ui.home.HomeFragment;
import com.example.friends.ui.home.HomeListAdapter;
import com.example.friends.ui.home.MessageHome;
...
public class FriendsActivity extends AppCompatActivity {
  ...
  private List<MessagesFriends> mlist = new ArrayList<>();//list数据集
  private MessageFriendDAL friendDAL = new MessageFriendDAL();//数据实体类
                ...
                if (!TextUtils.isEmpty(message)) {//判断是否为空
                  ...
                  if (list != null) {//返回的list如果不为空
                    for (int i = 0; i < list.size(); i++) {//遍历
                      ...
                      istrue =
                              addtext(
                                      (String) obj[0], (String) obj[1], (String) obj[2], str);//传递数据判断是否数据为更改
  ...
  private Handler handler = new Handler();//创建handler进行异步消息处理
