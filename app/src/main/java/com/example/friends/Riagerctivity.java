...
public class Riagerctivity extends AppCompatActivity {
    ...
    private final LoginDAL login = new LoginDAL(); // 实例化数据库操作类
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_reages_yes.setOnClickListener(
                new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        // 判断所有的输入框中任意一个都不能为空
                        if (edit_reages_pwd.getText() == null
                                || edit_reages_phone.getText() == null
                                || edit_reages_nickname.getText() == null
                                || edit_reages_nickname.getText().equals("")
                                || edit_reages_phone.getText().equals("")
                                || edit_reages_pwd.equals("")) {
                            // 错误消息提示
                                    ...
                                    new Runnable() {
                                        ...
                                        public void run() {
                                            ...
                                            if (login.Bool(edit_reages_phone.getText().toString())) {
                                                ...
                                                int i =
                                                        login.regies(
                                                                edit_reages_phone.getText().toString(),
                                                                edit_reages_nickname.getText().toString(),
                                                                edit_reages_pwd.getText().toString());
                                                if (i > 0) { // 判断是否注册成功
                                                    Looper.prepare(); // 创建消息队列
                                                    // 界面提示信息
        ...
        // 取消注册按钮事件
        btn_reages_no.setOnClickListener(
                new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
